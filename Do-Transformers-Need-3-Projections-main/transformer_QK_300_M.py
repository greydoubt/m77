"""
Complete GPT Training Pipeline with KV Attention (Q = K)
Modified to use locally downloaded SlimPajama data
Single file, ready to run on 4× A100 80GB

This implements the KV attention variant where Query projection is removed
and K is used for both Query and Key (symmetric attention).

Usage:
    python train_kv_local_data.py
...
class TrainingConfig:
    ...
    # System
    seed: int = 42
    output_dir: str = "./outputs_kv"
    
    # ClearML
    clearml_project: str = "GPT-QKV-Comparison"
    clearml_task: str = "GPT-300M-KV-10B"  # CHANGED: Updated task name


# ============================================================================
# MODEL: KV ATTENTION (Q = K, SYMMETRIC ATTENTION)
# ============================================================================

class KVAttention(nn.Module):
    """
    Multi-head KV attention where Q = K (symmetric attention)
    
    Key difference from QKV:
    - Only 2 projections instead of 3 (no separate Q projection)
    - Q = K, so attention matrix is symmetric before softmax: K @ K^T
    - This means: attention from token i→j equals attention from j→i (before softmax)
    ...
    def __init__(self, config: ModelConfig):
        ...
        # KV projection: Only 2× parameters (K and V, NO separate Q!)
        # This is the key difference: Q = K
        self.c_kv = nn.Linear(config.n_embd, 2 * config.n_embd, bias=config.use_bias)
        
        # Output projection (same as QKV)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Calculate K, V (NO separate Q!)
        kv = self.c_kv(x)  # [B, T, 2*C]
        k, v = kv.split(self.n_embd, dim=2)  # Each: [B, T, C]
        
        # Q = K (this is the KEY difference!)
        q = k
        ...
        if self.use_flash:
            ...
            # Note: q and k are the same tensor, so K @ K^T is computed
            out = flash_attn_func(
                q, k, v,
            ...
            # Note: This computes K @ K^T which is SYMMETRIC!
...
class MLP(nn.Module):
    """Feed-forward network (identical to QKV version)"""
...
class TransformerBlock(nn.Module):
    """Single transformer block with KV attention"""
    
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.ln_1 = nn.LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        self.attn = KVAttention(config)  # KV attention instead of QKV!
...
class GPT_KV(nn.Module):
    """GPT Language Model with KV attention (Q = K)"""
    
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.config = config
        
        # Token and position embeddings (identical to QKV)
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        self.drop = nn.Dropout(config.embd_pdrop)
        
        # Transformer blocks with KV attention
...
def train():
    ...
    if is_main_process:
        print("=" * 80)
        print("INITIALIZING KV MODEL (Q = K, Symmetric Attention)")
        print("=" * 80)
    
    model = GPT_KV(model_config).to(device)
    ...
    if is_main_process:
        ...
        print(f"\n⚠️  NOTE: KV attention has ~25% fewer attention parameters than QKV")
        print(f"   (2 projections instead of 3: K, V only - no separate Q)")
    ...
    if is_main_process:
        ...
        print("=" * 80)
        print("⚠️  Training KV model where Q = K (symmetric attention)")
        print("   Expect slightly worse perplexity than QKV (~18-19 vs ~17-18)")
    ...
    if is_main_process:
        ...
        print(f"\n⚠️  Expected: KV perplexity ~18-19 (vs QKV ~17-18)")
        print(f"   KV has symmetric attention (Q=K), which is less expressive")
