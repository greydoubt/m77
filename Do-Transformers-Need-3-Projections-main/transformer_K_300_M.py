"""
Complete GPT Training Pipeline with K Attention (Q = K = V)
Modified to use locally downloaded SlimPajama data
Single file, ready to run on 4× A100 80GB

This implements the K attention variant where ALL three projections (Q, K, V) 
are replaced with a SINGLE projection K. This is the most parameter-efficient
variant but also the most constrained.

Q = K = V (all use the same projection)

Usage:
    python train_k_local_data.py
...
from torch.utils.data import DataLoader, Dataset  # CHANGED: Dataset instead of IterableDataset
...
from datasets import load_from_disk  # CHANGED: Load from disk instead of streaming
...
class TrainingConfig:
    ...
    # System
    seed: int = 42
    output_dir: str = "./outputs_k"
    
    # ClearML
    clearml_project: str = "GPT-QKV-Comparison"
    clearml_task: str = "GPT-300M-K-10B"  # CHANGED: Updated task name


# ============================================================================
# MODEL: K ATTENTION (Q = K = V, SINGLE PROJECTION)
# ============================================================================

class KAttention(nn.Module):
    """
    Multi-head K attention where Q = K = V (single projection)
    
    This is the MOST parameter-efficient variant:
    - Only 1 projection instead of 3 (66% fewer parameters!)
    - Q = K = V, all use the same projection
    - Attention matrix is symmetric: K @ K^T
    - Values are also the same as keys
    
    Expected performance: Significantly worse than QKV and KV
    Paper shows ~15-20% degradation on complex tasks
    """
    ...
    def __init__(self, config: ModelConfig):
        ...
        # K projection: ONLY 1 projection! (Q = K = V = this single projection)
        # This is the most extreme reduction
        self.c_k = nn.Linear(config.n_embd, config.n_embd, bias=config.use_bias)
        
        # Output projection (same as QKV and KV)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_flash:
            ...
            # Note: q, k, v are all the same tensor!
            out = flash_attn_func(
                q, k, v,
            ...
            # This computes K @ K^T which is SYMMETRIC!
            ...
            # Softmax (breaks symmetry)
            attn = F.softmax(attn, dim=-1)
            attn = self.attn_dropout(attn)
            
            # Apply attention to values (which are also K!)
...
class MLP(nn.Module):
    """Feed-forward network (identical to QKV and KV versions)"""
...
class TransformerBlock(nn.Module):
    """Single transformer block with K attention"""
    
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.ln_1 = nn.LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        self.attn = KAttention(config)  # K attention (Q = K = V)
...
class GPT_K(nn.Module):
    """GPT Language Model with K attention (Q = K = V)"""
    
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.config = config
        
        # Token and position embeddings (identical to QKV and KV)
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        self.drop = nn.Dropout(config.embd_pdrop)
        
        # Transformer blocks with K attention
...
class LocalSlimPajamaDataset(Dataset):  # CHANGED: Regular Dataset instead of IterableDataset
    """Dataset from locally downloaded SlimPajama data"""
    
    def __init__(
        self,
        data_path: str,  # CHANGED: Takes path instead of split name
        tokenizer,
        seq_length: int = 2048,
        max_examples: Optional[int] = None
    ):
        print(f"Loading dataset from {data_path}...")
        self.dataset = load_from_disk(data_path)  # CHANGED: Load from disk
        ...
        # ADDED: Pre-compute total tokens for progress tracking
    ...
    def __len__(self):  # ADDED: Can now get length
        return len(self.dataset)
    
    def __getitem__(self, idx):  # CHANGED: Use __getitem__ instead of __iter__
...
def get_dataloader(
    ...
    shuffle: bool = True,  # ADDED: Shuffle option
    max_examples: Optional[int] = None
) -> DataLoader:
    ...
    return DataLoader(
        ...
        shuffle=shuffle,  # ADDED: Now can shuffle
        num_workers=num_workers,
        pin_memory=True,
        prefetch_factor=prefetch_factor,
        persistent_workers=True if num_workers > 0 else False,
        drop_last=True  # ADDED: Drop last incomplete batch
    )
...
def train():
    ...
    if is_main_process:
        print("=" * 80)
        print("INITIALIZING K MODEL (Q = K = V, Single Projection)")
        print("=" * 80)
        print("⚠️  WARNING: This is the most constrained variant!")
        print("   Expected to perform significantly worse than QKV and KV")
    
    model = GPT_K(model_config).to(device)
    ...
    if is_main_process:
        ...
        print(f"\n⚠️  NOTE: K attention has ~50% fewer attention parameters than QKV")
        print(f"   (1 projection instead of 3: only K, where Q=K=V)")
        print(f"   QKV: 84M attention params")
        print(f"   KV:  63M attention params")
        print(f"   K:   42M attention params (this model)")
    ...
    # ADDED: Check if data paths exist
    if is_main_process:
    ...
    train_loader = get_dataloader(
        data_path=train_config.train_data_path,  # CHANGED: Use path
        tokenizer=tokenizer,
        batch_size=train_config.micro_batch_size,
        seq_length=model_config.n_positions,
        num_workers=train_config.num_workers,
        prefetch_factor=train_config.prefetch_factor,
        shuffle=True  # ADDED: Shuffle training data
    )
    
    val_loader = get_dataloader(
        data_path=train_config.val_data_path,  # CHANGED: Use path
        tokenizer=tokenizer,
        batch_size=train_config.micro_batch_size,
        seq_length=model_config.n_positions,
        num_workers=train_config.num_workers,
        prefetch_factor=train_config.prefetch_factor,
        shuffle=False  # ADDED: Don't shuffle validation
    )
    ...
    if is_main_process:
        ...
        print("=" * 80)
        print("⚠️  Training K model where Q = K = V (single projection)")
        print("   Expected perplexity: ~20-24 (significantly worse than QKV ~17-18)")
        print("   Paper shows K performs much worse on NLP tasks")
    ...
    while step < total_steps:
        ...
        for micro_step in range(train_config.gradient_accumulation):
            try:
                ...
                # New epoch
                epoch += 1  # ADDED: Increment epoch
                if is_main_process:  # ADDED: Log new epoch
        ...
        if is_main_process and step % train_config.log_interval == 0:
            ...
            print(
                ...
                f"Epoch {epoch} | "  # ADDED: Show epoch
            ...
            if task:
                ...
                task.get_logger().report_scalar(
                    "train", "epoch", iteration=step, value=epoch  # ADDED: Log epoch
                )
        ...
        if is_main_process and step % train_config.eval_interval == 0:
            ...
            val_metrics = evaluate(
                model,
                val_loader,
                max_batches=eval_batches,  # CHANGED: Use batch count
                device=device
            )
    ...
    if is_main_process:
        ...
        print(f"\n⚠️  Expected comparison:")
        print(f"   QKV: ~17-18 perplexity")
        print(f"   KV:  ~18-19 perplexity")
        print(f"   K:   ~20-24 perplexity (this model)")
        print(f"\n   K has Q=K=V constraint, making it least expressive")
