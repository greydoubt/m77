"""
Complete GPT Training Pipeline with Grouped-Query Attention (GQA)
...
- Reduced K,V projections with n_kv_groups heads (4 in this case)
...
Usage:
    torchrun --nproc_per_node=4 train_gqa_local_data.py
...
class ModelConfig:
    ...
    n_embd: int = 1024
    n_head: int = 16
    n_kv_groups: int = 4  # NEW: Number of KV groups (GQA-4 like Llama 2)
    n_inner: int = 4096
...
class TrainingConfig:
    ...
    # System
    seed: int = 42
    output_dir: str = "./outputs_gqa"
    
    # ClearML
    clearml_project: str = "GPT-QKV-Comparison"
    clearml_task: str = "GPT-300M-GQA-4-10B"
...
class GQAttention(nn.Module):
    ...
    - Reduced K,V projections with n_kv_groups heads (e.g., 4 groups)
    - Each KV group is shared by n_head/n_kv_groups query heads
    ...
    - 4 value heads (each shared by 4 query heads)
    - Cache stores 4 KV heads instead of 16 (75% reduction!)
    """
...
def train():
    ...
    if is_main_process:
        print("=" * 80)
        print("INITIALIZING GQA MODEL (Grouped-Query Attention)")
        print("=" * 80)
    ...
    if is_main_process:
        ...
        print(f"   GQA-4 (this model): {kv_cache_at_2k:.2f} MB @ 2k, {kv_cache_at_32k:.2f} GB @ 32k")
        print(f"   Cache reduction: {(1 - model_config.n_kv_groups/model_config.n_head)*100:.0f}%")
        print(f"\n💡 Expected: GQA quality ~5.15-5.25 PPL (between QKV and Q-KV)")
    ...
    if is_main_process:
        ...
        print("=" * 80)
        print("🔬 HYPOTHESIS TEST: GQA vs projection-sharing variants")
        print("   Expected: GQA quality similar to QKV/Q-KV but with better cache efficiency")
    ...
    if is_main_process:
        ...
        print(f"   Q-KV (K=V): 5.27 PPL | 1.31 GB cache @ 32k")
        print(f"   GQA-4: {val_metrics['perplexity']:.2f} PPL | {kv_cache_at_32k:.2f} GB cache @ 32k")
