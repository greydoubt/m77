"""
Complete GPT Training Pipeline with Q-GQA Attention (Hybrid: Grouped-Query + K=V)
Modified to use locally downloaded SlimPajama data
Single file, ready to run on 4× A100 80GB

Q-GQA combines two cache-saving techniques:
1. Grouped queries (like GQA): Only n_kv_groups K heads instead of n_head
2. K = V constraint (like Q-KV): No separate V projection, V reuses K

Result: MAXIMUM KV cache savings!
- 16 Q heads (full expressiveness)
- 4 K heads (grouped)
- V = K (no separate projection)
- Cache: Only 4 K heads = 87.5% reduction vs standard QKV!

Usage:
    torchrun --nproc_per_node=4 train_q_gqa_local_data.py
...
class TrainingConfig:
    ...
    # System
    seed: int = 42
    output_dir: str = "./outputs_q_gqa"
    
    # ClearML
    clearml_project: str = "GPT-QKV-Comparison"
    clearml_task: str = "GPT-300M-Q-GQA-4-10B"


# ============================================================================
# MODEL: Q-GQA ATTENTION (Hybrid: Grouped-Query + K=V)
# ============================================================================

class QGQAttention(nn.Module):
    """
    Q-GQA: Hybrid of Grouped-Query Attention + K=V constraint
    
    This is the ULTIMATE cache-saving attention mechanism!
    
    Key characteristics:
    - Full Q projection with n_head heads (e.g., 16 heads) - maintains expressiveness
    - Reduced K projection with n_kv_groups heads (e.g., 4 groups) - GQA's trick
    - V = K (no separate V projection) - Q-KV's trick
    - Each K group is shared by n_head/n_kv_groups query heads AND used as V
    
    Benefits:
    - 87.5% KV cache reduction vs standard QKV (16 K + 16 V → 4 K only!)
    - 75% reduction vs GQA (4 K + 4 V → 4 K only)
    - 50% reduction vs Q-KV (16 K → 4 K)
    - Maintains asymmetric attention (Q independent)
    - Production-viable combination of proven techniques
    
    Example with n_head=16, n_kv_groups=4:
    - 16 separate query heads (full expressiveness)
    - 4 key heads (each shared by 4 query heads)
    - V = K, so no additional projection or cache needed
    - Cache: 4 K heads only (vs 16 K + 16 V in QKV)
    
    Expected quality: Between GQA and Q-KV, likely ~5.3-5.5 PPL
    - Better than K model (6.41 PPL) due to independent Q
    - Possibly slightly worse than GQA (~5.15-5.25 PPL) due to K=V constraint
    - But with MAXIMUM cache savings!
    """
    ...
    def __init__(self, config: ModelConfig):
        ...
        # REDUCED K projection (only n_kv_groups heads)
        # This is the KEY difference: combining GQA's grouping with Q-KV's K=V
        k_dim = self.n_kv_groups * self.head_dim
        self.c_k = nn.Linear(config.n_embd, k_dim, bias=config.use_bias)
        # NO c_v projection! V will reuse K
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Key projection: Only n_kv_groups heads (GROUPED like GQA)
        k = self.c_k(x)  # [B, T, n_kv_groups * head_dim]
        k = k.view(B, T, self.n_kv_groups, self.head_dim)  # [B, T, n_kv_groups, head_dim]
        
        # V = K (NO separate projection, like Q-KV model!)
        # This is the SECOND key difference
        v = k  # [B, T, n_kv_groups, head_dim]
        
        # Repeat K and V heads to match number of query heads
        # Each KV group is used by n_head_per_group query heads
        # Example: [B, T, 4, head_dim] -> [B, T, 16, head_dim] with each group repeated 4 times
        k = k.repeat_interleave(self.n_head_per_group, dim=2)  # [B, T, n_head, head_dim]
        v = v.repeat_interleave(self.n_head_per_group, dim=2)  # [B, T, n_head, head_dim]
        
        # Now q, k, v all have shape [B, T, n_head, head_dim]
        # But k and v:
        #   1. Came from only n_kv_groups projections (GQA's trick)
        #   2. Are the same tensor (Q-KV's trick)
        # Result: MAXIMUM cache savings!
        ...
        if self.use_flash:
            ...
            # Q @ K^T is NOT symmetric (Q independent)
            # This gives better expressiveness than K model (Q=K=V)
...
class TransformerBlock(nn.Module):
    """Single transformer block with Q-GQA"""
    
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.ln_1 = nn.LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        self.attn = QGQAttention(config)  # Q-GQA: Grouped + K=V!
...
class GPT_Q_GQA(nn.Module):
    """GPT Language Model with Q-GQA Attention (Hybrid: Grouped-Query + K=V)"""
...
def train():
    ...
    if is_main_process:
        print("=" * 80)
        print("INITIALIZING Q-GQA MODEL (Hybrid: Grouped-Query + K=V)")
        print("=" * 80)
        print("🚀 ULTIMATE CACHE OPTIMIZATION: Combining TWO proven techniques!")
        print(f"   • {model_config.n_head} query heads (full expressiveness)")
        print(f"   • {model_config.n_kv_groups} key groups (GQA's grouping)")
        print(f"   • V = K (Q-KV's constraint)")
        print(f"   • Each K group shared by {model_config.n_head // model_config.n_kv_groups} Q heads AND used as V")
        print(f"   • Result: MAXIMUM cache savings!")
    
    model = GPT_Q_GQA(model_config).to(device)
    ...
    if is_main_process:
        ...
        # Calculate KV cache sizes for comparison
        head_dim = model_config.n_embd // model_config.n_head
        bytes_per_element = 2  # FP16
        
        # QKV: 16 K + 16 V = 32 heads total
        qkv_cache_per_token = 2 * model_config.n_head * model_config.n_layer * head_dim * bytes_per_element
        qkv_cache_2k = qkv_cache_per_token * 2048 / (1024**2)
        qkv_cache_32k = qkv_cache_per_token * 32768 / (1024**2)
        
        # GQA: 4 K + 4 V = 8 heads total
        gqa_cache_per_token = 2 * model_config.n_kv_groups * model_config.n_layer * head_dim * bytes_per_element
        gqa_cache_2k = gqa_cache_per_token * 2048 / (1024**2)
        gqa_cache_32k = gqa_cache_per_token * 32768 / (1024**2)
        
        # Q-KV: 16 K (no V) = 16 heads total
        qkv_kequalsv_cache_per_token = model_config.n_head * model_config.n_layer * head_dim * bytes_per_element
        qkv_kequalsv_cache_2k = qkv_kequalsv_cache_per_token * 2048 / (1024**2)
        qkv_kequalsv_cache_32k = qkv_kequalsv_cache_per_token * 32768 / (1024**2)
        
        # Q-GQA: 4 K (no V) = 4 heads total ← MINIMUM!
        qgqa_cache_per_token = model_config.n_kv_groups * model_config.n_layer * head_dim * bytes_per_element
        qgqa_cache_2k = qgqa_cache_per_token * 2048 / (1024**2)
        qgqa_cache_32k = qgqa_cache_per_token * 32768 / (1024**2)
        
        print(f"\n📊 KV CACHE COMPARISON:")
        print(f"   QKV (16 K + 16 V):     {qkv_cache_2k:.2f} MB @ 2k,  {qkv_cache_32k:.2f} GB @ 32k")
        print(f"   GQA-4 (4 K + 4 V):     {gqa_cache_2k:.2f} MB @ 2k,  {gqa_cache_32k:.2f} GB @ 32k")
        print(f"   Q-KV (16 K, V=K):      {qkv_kequalsv_cache_2k:.2f} MB @ 2k,  {qkv_kequalsv_cache_32k:.2f} GB @ 32k")
        print(f"   Q-GQA (4 K, V=K): {qgqa_cache_2k:.2f} MB @ 2k, {qgqa_cache_32k:.2f} GB @ 32k ⭐ MINIMUM!")
        print(f"\n   Cache reduction vs QKV: {(1 - qgqa_cache_32k/qkv_cache_32k)*100:.1f}%")
        print(f"   Cache reduction vs GQA: {(1 - qgqa_cache_32k/gqa_cache_32k)*100:.1f}%")
        print(f"   Cache reduction vs Q-KV: {(1 - qgqa_cache_32k/qkv_kequalsv_cache_32k)*100:.1f}%")
        
        print(f"\n💡 Expected quality: ~5.3-5.5 PPL")
        print(f"   (Between GQA and Q-KV, trading some quality for MAXIMUM cache efficiency)")
        ...
        if task:
            ...
            task.get_logger().report_single_value("model/kv_cache_gb_at_32k", qgqa_cache_32k)
            task.get_logger().report_single_value("model/cache_reduction_vs_qkv_percent", (1 - qgqa_cache_32k/qkv_cache_32k)*100)
    ...
    if is_main_process:
        ...
        print("=" * 80)
        print("🔬 HYPOTHESIS TEST: Q-GQA (ultimate cache optimization)")
        print("   Combines: GQA's grouping (75% reduction) + Q-KV's K=V (50% reduction)")
        print("   Result: 87.5% cache reduction vs QKV!")
        print("   Expected: Quality ~5.3-5.5 PPL (slight tradeoff for MAXIMUM efficiency)")
    ...
    if is_main_process:
        ...
        print(f"\n🎯 RESULTS COMPARISON:")
        print(f"   QKV (baseline):    5.11 PPL | 2.62 GB cache @ 32k | 83.97M attn params")
        print(f"   KV (Q=K):          5.36 PPL | 2.62 GB cache @ 32k | 62.98M attn params")
        print(f"   GQA-4:        ~5.15-5.25 PPL | 0.66 GB cache @ 32k | 41.98M attn params")
        print(f"   Q-KV (K=V):   ~5.20-5.40 PPL | 1.31 GB cache @ 32k | 62.98M attn params")
        print(f"   Q-GQA-4:      {val_metrics['perplexity']:.2f} PPL | {qgqa_cache_32k:.2f} GB cache @ 32k | {params['attention_M']:.2f}M attn params ⭐")
        print(f"   K (Q=K=V):         6.41 PPL | 1.31 GB cache @ 32k | 41.98M attn params")
        
        cache_vs_qkv = (1 - qgqa_cache_32k/qkv_cache_32k) * 100
        cache_vs_gqa = (1 - qgqa_cache_32k/gqa_cache_32k) * 100
        
        print(f"\n📊 Q-GQA ACHIEVEMENTS:")
        print(f"   ✓ {cache_vs_qkv:.1f}% cache reduction vs QKV")
        print(f"   ✓ {cache_vs_gqa:.1f}% cache reduction vs GQA")
        print(f"   ✓ {params['attention_M']:.2f}M attention params (vs 83.97M QKV)")
        
        if val_metrics['perplexity'] < 5.6:
            print(f"\n🏆 SUCCESS! Q-GQA achieves competitive quality with MAXIMUM cache efficiency!")
            print(f"   This is the MOST cache-efficient variant that maintains good quality!")
        else:
            print(f"\n⚠️  Q-GQA quality is lower. The double constraint (grouping + K=V) impacts performance.")
