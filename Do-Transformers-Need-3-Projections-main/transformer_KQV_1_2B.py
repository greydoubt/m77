"""
Complete GPT Training Pipeline with QKV Attention - 1.2B Model
Modified to use locally downloaded SlimPajama data
Distributed training on 8× A100 40GB - OPTIMIZED

Usage:
    torchrun --nproc_per_node=8 transformer_KQV_1.2B.py
...
class TrainingConfig:
    ...
    micro_batch_size: int = 2          # ← OPTIMIZED: 2 → 3
    gradient_accumulation: int = 36    # ← OPTIMIZED: 36 → 24
    ...
    # System
    seed: int = 42
    output_dir: str = "./outputs_1.2B_kqv"
    
    # ClearML
    clearml_project: str = "GPT-QKV-Comparison"
    clearml_task: str = "GPT-1.2B-QKV-10B"
...
def train():
    ...
    # ← OPTIMIZATION #2: Compile model (PyTorch 2.0+)
    ...
    optimizer = torch.optim.AdamW(
        ...
        fused=True  # ← OPTIMIZATION: Fused AdamW (5-10% faster)
    )
