"""
Complete GPT Training Pipeline with Q-KV Attention (K = V)
Modified to use locally downloaded SlimPajama data
Single file, ready to run on 4× A100 80GB

This implements the Q-KV attention variant where Value projection is removed
and K is used for both Key and Value.

KEY BENEFITS:
- Independent Q projection (asymmetric attention unlike KV model)
- 50% KV cache reduction (only need to cache K, since V=K)
- Same parameter count as KV model (62.98M attention params)
- Expected better quality than K model, similar to KV model

Usage:
    torchrun --nproc_per_node=8 train_qkv_k_equals_v.py
...
class TrainingConfig:
    ...
    # System
    seed: int = 42
    output_dir: str = "./outputs_qkv_k_equals_v"
    
    # ClearML
    clearml_project: str = "GPT-QKV-Comparison"
    clearml_task: str = "GPT-300M-QKV-KEqualsV-10B"


# ============================================================================
# MODEL: Q-KV ATTENTION (K = V, Q INDEPENDENT)
# ============================================================================

class QKVAttention_KEqualsV(nn.Module):
    """
    Multi-head Q-KV attention where K = V (Q is independent)
    
    Key characteristics:
    - Only 2 projections instead of 3 (Q and K, NO separate V!)
    - V = K, so we only cache K during inference (50% cache reduction!)
    - Q is independent, so attention is NOT symmetric: Q @ K^T != K @ K^T
    - This maintains more expressiveness than K model (Q=K=V) or KV model (Q=K)
    
    Expected benefits:
    - Better quality than K model (Q independent allows asymmetric attention)
    - Similar quality to KV model (both have 2 projections)
    - 50% KV cache reduction (only cache K, V=K)
    - Same parameter count as KV model (62.98M attention params)
    
    This is the "missing variant" that combines:
    - Asymmetric attention from having independent Q
    - Memory efficiency from V=K constraint
    """
    ...
    def __init__(self, config: ModelConfig):
        ...
        # Q and K projections: Only 2× parameters (Q and K, NO separate V!)
        # This is the key difference: V = K
        self.c_q = nn.Linear(config.n_embd, config.n_embd, bias=config.use_bias)
        self.c_k = nn.Linear(config.n_embd, config.n_embd, bias=config.use_bias)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Calculate Q and K (NO separate V!)
        q = self.c_q(x)  # [B, T, C] - Independent query projection
        k = self.c_k(x)  # [B, T, C] - Key projection
        
        # V = K (this is the KEY difference!)
        # This means we apply attention weights to K itself
        v = k
        ...
        if self.use_flash:
            ...
            # Note: q and k are different tensors (NOT symmetric attention!)
            # But v = k, so output is attention-weighted version of keys
            out = flash_attn_func(
                q, k, v,
            ...
            # This computes Q @ K^T which is NOT SYMMETRIC (unlike KV model)
            # This is good! It means token i can attend to token j differently
            # than token j attends to token i
            ...
            # This means: "attend to tokens, get their key representations"
...
class TransformerBlock(nn.Module):
    """Single transformer block with Q-KV attention (K=V)"""
    
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.ln_1 = nn.LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        self.attn = QKVAttention_KEqualsV(config)  # Q-KV attention (K=V)
...
class GPT_QKV_KEqualsV(nn.Module):
    """GPT Language Model with Q-KV attention (K = V)"""
    ...
    def __init__(self, config: ModelConfig):
        ...
        # Transformer blocks with Q-KV attention
...
def train():
    ...
    if is_main_process:
        print("=" * 80)
        print("INITIALIZING Q-KV MODEL (K = V, Q Independent)")
        print("=" * 80)
        print("⭐ THE MISSING VARIANT: Combines benefits of both worlds!")
        print("   • Q is independent → Asymmetric attention (more expressive)")
        print("   • V = K → 50% KV cache reduction (memory efficient)")
    
    model = GPT_QKV_KEqualsV(model_config).to(device)
    ...
    if is_main_process:
        ...
        print(f"\n📊 COMPARISON:")
        print(f"   QKV: 83.97M attention params, 2.62 GB cache @ 32k")
        print(f"   KV (Q=K): 62.98M attention params, 2.62 GB cache @ 32k")
        print(f"   Q-KV (K=V): {params['attention_M']:.2f}M attention params, 1.31 GB cache @ 32k ⭐")
        print(f"   K (Q=K=V): 41.98M attention params, 1.31 GB cache @ 32k")
        print(f"\n💡 Expected: Q-KV quality ~5.2-5.4 PPL (between KV and K models)")
        print(f"   Better than K model due to asymmetric attention")
        print(f"   Similar to KV model but with 50% less cache!")
    ...
    if is_main_process:
        ...
        print("=" * 80)
        print("🔬 HYPOTHESIS TEST: Q-KV (K=V) performance")
        print("   Expected: Quality similar to KV (~5.2-5.4 PPL)")
        print("   Bonus: 50% KV cache reduction (only cache K)")
        print("   Why: Q independence → asymmetric attention → better than K model")
    ...
    if is_main_process:
        ...
        print(f"\n🎯 RESULTS COMPARISON:")
        print(f"   QKV (baseline): 5.11 PPL | 2.62 GB cache @ 32k")
        print(f"   KV (Q=K): 5.36 PPL | 2.62 GB cache @ 32k")
        print(f"   Q-KV (K=V): {val_metrics['perplexity']:.2f} PPL | 1.31 GB cache @ 32k")
        print(f"   K (Q=K=V): 6.41 PPL | 1.31 GB cache @ 32k")
        
        if val_metrics['perplexity'] < 5.5:
            print(f"\n🏆 SUCCESS! Q-KV achieved competitive quality with 50% cache reduction!")
        else:
            print(f"\n⚠️  Q-KV quality lower than expected. K=V constraint may hurt more than Q=K.")
